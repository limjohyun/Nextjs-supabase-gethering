...
- **context7**: 어떤 설정·의존성이 최신 Next.js 15.5.3/React 19 컨벤션에 맞는지 확신이 안 서면 추측하지 말고 `resolve-library-id` → `query-docs`로 먼저 확인한다.
- **supabase MCP**: `.env`나 코드에 남아있는 Supabase 관련 설정을 지우기 전에 `list_tables`/`list_extensions`로 실제 프로젝트에 연결된 스키마가 있는지 확인해, 실사용 중인 설정을 실수로 제거하지 않는다.
- **shrimp-task-manager**: 6단계 최적화 프로세스처럼 여러 단계로 나뉘는 정리 작업은 이 도구로 각 단계를 작업으로 등록하고 진행 상태를 추적한다.

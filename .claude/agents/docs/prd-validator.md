...
### 🔌 검증에 사용할 MCP 서버 (WebFetch보다 우선)

- **context7**: 라이브러리/프레임워크(Next.js, Supabase JS, shadcn/ui 등) 공식 문서는 WebFetch로 페이지를 직접 긁기 전에 `resolve-library-id` → `query-docs`로 먼저 확인한다. 버전별 최신 문서를 구조화된 형태로 제공하므로 [FACT] 근거로 삼기에 더 적합하다.
- **supabase MCP**: PRD가 Supabase 기반 기능(인증, RLS, 실시간 등)을 전제한다면 `search_docs`로 공식 문서를 확인하고, 이미 존재하는 프로젝트라면 `list_tables`/`get_advisors`로 실제 스키마·보안 제약을 검증한다.
- **sequential-thinking**: 이 에이전트의 Chain of Thought 프로세스(Step 0~5)를 진행할 때 `mcp__sequential-thinking__sequentialthinking`으로 각 단계의 사고 과정을 명시적으로 기록하면 재검토와 자기 검증 루프가 더 견고해진다.
- **WebFetch**: 위 MCP 서버가 다루지 않는 문서(예: 특정 서드파티 API의 공식 페이지)는 WebFetch로 직접 확인한다.

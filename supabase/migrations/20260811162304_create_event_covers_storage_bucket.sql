...
create policy "누구나 event-covers 이미지를 조회할 수 있다"
on storage.objects
for select
to public
...
create policy "인증된 사용자는 본인 폴더에만 커버 이미지를 업로드할 수 있다"
on storage.objects
for insert
to authenticated
with check (
...
comment on policy "인증된 사용자는 본인 폴더에만 커버 이미지를 업로드할 수 있다"
  on storage.objects is 'event-covers 버킷에 대한 유일한 쓰기 경로. 파일 경로의
  최상위 폴더가 업로더 본인의 auth.uid()와 일치해야만 업로드를 허용한다.
  UPDATE/DELETE 정책은 의도적으로 두지 않았다(MVP 범위 외 — 기존 커버 이미지
